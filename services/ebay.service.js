...
async function fetchEbayRawProducts(keyword, maxPrice) {
  ...
  try {
    console.log(`>>> [eBayService] Requête eBay : "${keyword}" (maxPrice=${maxPrice})`);
    const res = await fetch(`${endpoint}?${params.toString()}`);
    const data = await res.json();
    const ack = data.findItemsByKeywordsResponse?.[0]?.ack?.[0];
    if (ack !== 'Success') {
      console.error(">>> [eBayService] Réponse eBay invalide :", JSON.stringify(data));
      return [];
    }
...
// Application des règles de filtrage + scoring
function applyEbayBusinessRules(products, data) {
  ...
  return products.map(item => {
    ...
    if (excluded.some(e => title.includes(e))) {
      console.log(`>>> [eBayService] Écarté (exclu) : ${title}`);
      return null;
    }

    let matchingScore = scoringConfig.BASE_SCORE;
    console.log(`>>> [eBayService] ${title} → base : ${matchingScore}%`);
    ...
    if (foundKeywords.length >= 3) {
      ...
      console.log(`>>> [eBayService] +${scoringConfig.ADVANCED_MATCH_BONUS}% : compatibilité avancée (${foundKeywords.length})`);
    }
    ...
    if (originalPrice > price) {
      ...
      console.log(`>>> [eBayService] +${promoBonus}% : promotion`);
    }
    ...
    if (isFast) {
      ...
      console.log(`>>> [eBayService] +${fastBonus}% : livraison rapide`);
    }
    ...
    if (!isNaN(weightKg) && weightKg <= 5) {
      ...
      console.log(`>>> [eBayService] +${compactBonus}% : format compact`);
    }
...
async function searchEbayProducts(data) {
  try {
    ...
    console.log(`>>> [eBayService] ${filtered.length} produits retenus pour "${interest}"`);
    return filtered;
  } catch (err) {
